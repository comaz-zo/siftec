...
function Leaflet_initMap()
{
    ...
    //L_CP_UFOR.addTo(L_MAP);
   
    //L_CP_PARTICELLARE.addTo(L_MAP);
    //L_CP_PARTICELLARE_ETICHETTE.addTo(L_MAP);
	
	
    //L_CP_VIABILITA.addTo(L_MAP);
    
    //L_CP_TOPONIMI.addTo(L_MAP);
    
    //L_CP_GEONOTE.addTo(L_MAP);
    ...
    L_MAP_LAYERS = new L.control.layers(baseMaps, overlayMaps, {
        //'autoZIndex': false
    });
    ...
    /*L_MAP_PANELLAYERS = new L.control.panelLayers(
    [
        {
            group: "Base layers",
            layers: [
        ...
        
        {
            ...
            layers: [
                ...
                },
                {
                    active: false,
                    name: "Particellare",
                    layer: L_CP_PARTICELLARE
                },         
            ]
        },
        {
            group: "Unità Forestali",
            layers: [
                {
                    active: false,
                    name: "Unità forestali",
                    layer: L_CP_UFOR
                } 
            ]
        }   
    ...
    L_MAP.locate({watch:true, setView: false, maxZoom: 16});
...
//
// CAMBIO STILE IN BASE ALLA PAGINA
//

/* RIMAPPATA SU UFOR
function goToMap_STRATI(map_name){
    console.log(MAP_ACTIVE);
    if (MAP_ACTIVE!="STRATI"){
        Leaflet_initMap_STRATI();
        MAP_ACTIVE="STRATI";
        console.log(MAP_ACTIVE);
    }
    ...
    LEGEND_CTRL.onAdd = function (L_MAP) {
        ...
        var div = L.DomUtil.create('div', 'info legend'),
            grades = ["P","QL","S","PS", "AGR","M", "PAS", "ACQ",  "IMP",   "URB"],
            labels = [
                "Pineta",
                "Querceto e boschi misti di latifoglie",
                "Sughereta",
                "Piantagioni speciali",
                "Zone agrarie",
                "Macchia mediterranea",
                "Pascoli",
                "Abbeveratoi e stagni",
        ...
        // loop through our density intervals and generate a label with a colored square for each interval
        for (var i = 0; i < grades.length; i++) {
            div.innerHTML += '<i style="background:' + STRATI_getColor(grades[i]) + '"></i> ' + labels[i] + (grades[i + 1] ? '<br>' : '');
        }
    ...
    if (!L_MAP.hasLayer(L_CP_STRATI)){
        L_MAP.addLayer(L_CP_STRATI);
    }
...
function UFOR_goToFeature(e) {
    ...
    $("#ufor_page_header").text('UFOR: '+layer.feature.properties.K_UFOR);
    ...
    var comp_button = '<br><a href="#" id="comp_button" onclick="ufor_comp_open_panel(\''+layer.feature.properties.K_UFOR+'\')" data-role="button"  data-theme="b" >Composizione</a>';

            
    var uforTable = '<b>UFOR: '+ layer.feature.properties.K_UFOR + '</b>'+comp_button+'<table data-role="table"  id="ufor-table"  class="ui-responsive table-stroke" style="text-align:left;padding:10px;"><thead><tr><th>Parametro</th><th>Valore</th></tr></thead><tbody>'+r.join('')+"</tbody></table>";
...
function ufor_comp_open_panel(key){
    ...
    r[++j] ='<tr>';
    r[++j] ='<td colspan="2">';
    var sch_button =  '<br><a href="#"  onclick="$.mobile.activePage.find(\'#ufor_panel\').panel(\'open\')" data-role="button" data-theme="b" >Scheda</a>';
    r[++j] = '</td>';
    r[++j] ='</tr>';
    ...
    var compTable = '<b>UFOR: '+ key + '</b>'+sch_button+'<table data-role="table" id="comp-table"  class="ui-responsive table-stroke" style="text-align:left;padding:10px;"><thead><tr><th>Specie</th><th>Cop STR1</th><th>Cop STR2</th></tr></thead><tbody>'+r.join('')+"</tbody></table>";
...
//             r[++j] ='</td>';
//             r[++j] ='</tr>';
            r[++j] ='<div class="ui-block-a"><a onclick="geonote_openPhoto(\'' + value + '\')"><img src=\"images\\foto_geonote\\' + value + '\" width="100%"></a></div>';
        }
    }
...
function geonote_openPhoto(e) {
    ...
    var photo = '<img src=\"images\\foto_geonote\\' + e + '\" width="100%">';
...
function BIGPLOT_goToFeature(e) {
    ...
    for(var i=1;i<=5;i++)
    {
        ...
        if (value)
        {
            img_array[++j] ='<div class="ui-block-a"><a onclick="bigplot_openPhoto(\'' + value + '\')"><img src=\"images\\foto_bigplot\\' + value + '\" width="70%"></a></div>';
        }
    }

    var bigplotTable = '<b>BIGPLOT: '+ layer.feature.properties.ID_BP + '</b>'+'<table data-role="table"  id="bigplot-table"  class="ui-responsive table-stroke" style="text-align:left;padding:10px;"><thead><tr><th>Parametro</th><th>Valore</th></tr></thead><tbody>'+r.join('')+"</tbody></table>"+'<div class="ui-grid-solo">'+img_array.join('')+"</div>";
...
function bigplot_openPhoto(e) {
    ...
    var photo = '<img src=\"images\\foto_bigplot\\' + e + '\" width="100%">';
