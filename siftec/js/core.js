...
function Leaflet_initlayers(){
    ...
    L_CP_PARTICELLARE = new L.geoJson(particellare, {
    ...
    L_CP_PARTICELLARE_ETICHETTE = new L.geoJson(particellare_etichette, {
    ...
    L_CP_GEONOTE_CLUSTER = L.markerClusterGroup({
    	//spiderfyOnMaxZoom: false,
    	showCoverageOnHover: false,
    	//zoomToBoundsOnClick: false
    });
    L_CP_GEONOTE_CLUSTER.addLayer(L_CP_GEONOTE);
...
function Leaflet_initMap()
{
    ...
    L_MAP.on('zoomend', function (e) {
        ...
        switch (current_zoom){
            case 13:
                /*if (L_MAP.hasLayer(L_CP_GEONOTE_CLUSTER)){
                    L_MAP.removeLayer(L_CP_GEONOTE_CLUSTER);
                }*/
...
function goToMap_UFOR(map_name){
    ...
    switch(map_name)
    {
        ...
        case "COLTURA":
                L_CP_UFOR.setStyle(UFOR_COLTURA_style);


                LEGEND_CTRL.onAdd = function (L_MAP) {

                var div = L.DomUtil.create('div', 'info legend'),
                    grades = [11,21,31,41,51,61,0],
                    labels = [
                        "Fustaie",
                        "Fustaie transitorie",
                        "Cedui",
                    ...
                    // loop through our density intervals and generate a label with a colored square for each interval
                    for (var i = 0; i < grades.length; i++) {
                        div.innerHTML += '<i style="background:' + UFOR_COLTURA_getColor(grades[i]) + '"></i> ' + labels[i] + (grades[i + 1] ? '<br>' : '');
                    }
        ...
        case "STRATI":
            L_CP_UFOR.setStyle(UFOR_STRATI_style);
            ...
            var div = L.DomUtil.create('div', 'info legend'),
                ...
                // loop through our density intervals and generate a label with a colored square for each interval
                for (var i = 0; i < grades.length; i++) {
                    div.innerHTML += '<i style="background:' + UFOR_STRATI_getColor(grades[i]) + '"></i> ' + labels[i] + (grades[i + 1] ? '<br>' : '');
                }
...
function removeBaseLayer(layer){
	if (L_MAP.hasLayer(layer)){
        L_MAP_LAYERS.removeLayer(layer);
        L_MAP.removeLayer(layer);
    }
}

function Leaflet_initMap_CLEAR_BASELAYERS()
{
    L_MAP.eachLayer(function(layer) {
        i=0;
    ...
    /*
    removeBaseLayer(L_OFD_2010);
    removeBaseLayer(L_DTM_HILLSHADE);
    removeBaseLayer(L_CHM_0_30);
    removeBaseLayer(L_OFD_1943);
    removeBaseLayer(L_OFD_1967);
    removeBaseLayer(L_OFD_1980);
    removeBaseLayer(L_OFD_1985);
    removeBaseLayer(L_OFD_1998);
    removeBaseLayer(L_VOLUMI);
    removeBaseLayer(L_BIOMASSE);
    */
    //
	// INIZIO OFD  LAYER
    //
    ...
    var L_OFD_2010 = new L.TileLayer.MBTiles('', {
        minZoom : L_MINZOOM,
        maxZoom : L_MAXZOOM,
        tms : true
    }, db_ofd_2010, "jpg");*/

    L_OFD_2010 = new L.tileLayer('./tiles/ofd_2010/{z}/{x}/{y}.jpg', {
      attribution: 'OFD Minambiente 2010'
    });

    L_OFD_2010.addTo(L_MAP);
    L_MAP_LAYERS.addBaseLayer(L_OFD_2010,"OFD");
...
function Leaflet_initMap_LIDAR()
{
    ...
    var L_DTM_HILLSHADE = new L.TileLayer.MBTiles('', {
        minZoom : L_MINZOOM,
        maxZoom : L_MAXZOOM,
        tms : true,
    ...
    var L_CHM_0_30 = new L.TileLayer.MBTiles('', {
        minZoom : L_MINZOOM,
        maxZoom : L_MAXZOOM,
        tms : true,
        attribution: 'CHM',
    ...
    */
    ...
    L_DTM_HILLSHADE = new L.tileLayer('./tiles/dtm_hillshade/{z}/{x}/{y}.png', {
      attribution: 'DTM Hillshade',
    });

    L_CHM_0_30 = new L.tileLayer('./tiles/chm_0_30/{z}/{x}/{y}.jpg', {
      attribution: 'CHM',
    });

    L_DTM_HILLSHADE.addTo(L_MAP);
    L_CHM_0_30.addTo(L_MAP);
...
function Leaflet_initMap_VOLUMI()
{
    ...
    var L_VOLUMI = new L.TileLayer.MBTiles('', {
        minZoom : L_MINZOOM,
        maxZoom : L_MAXZOOM,
        tms : true,
    ...
    */
    ...
    L_VOLUMI = new L.tileLayer('./tiles/volumi/{z}/{x}/{y}.png', {
      attribution: 'Volume/ha',
    });

    L_VOLUMI.addTo(L_MAP);
...
function Leaflet_initMap_BIOMASSE()
{
    ...
    var L_BIOMASSE = new L.TileLayer.MBTiles('', {
        minZoom : L_MINZOOM,
        maxZoom : L_MAXZOOM,
        tms : true,
    ...
    */
    L_BIOMASSE = new L.tileLayer('./tiles/biomasse/{z}/{x}/{y}.png', {
      attribution: 'Biomassa/ha',
    });

    L_BIOMASSE.addTo(L_MAP);
...
function Leaflet_initMap_OFD_STORICHE()
{
    ...
    var L_OFD_1943 = new L.TileLayer.MBTiles('', {
        minZoom : L_MINZOOM,
        maxZoom : L_MAXZOOM,
        tms : true,
    ...
    var L_OFD_1967 = new L.TileLayer.MBTiles('', {
        minZoom : L_MINZOOM,
        maxZoom : L_MAXZOOM,
        tms : true,
    ...
    var L_OFD_1980 = new L.TileLayer.MBTiles('', {
        minZoom : L_MINZOOM,
        maxZoom : L_MAXZOOM,
        tms : true,
    ...
    var L_OFD_1985 = new L.TileLayer.MBTiles('', {
        minZoom : L_MINZOOM,
        maxZoom : L_MAXZOOM,
        tms : true,
    ...
    var L_OFD_1998 = new L.TileLayer.MBTiles('', {
        minZoom : L_MINZOOM,
        maxZoom : L_MAXZOOM,
        tms : true,
    ...
    */
    ...
    L_OFD_1943 = new L.tileLayer('./tiles/ofd_1943/{z}/{x}/{y}.jpg', {
      attribution: 'Ortofoto 1943',
    });

    L_OFD_1967 = new L.tileLayer('./tiles/ofd_1967/{z}/{x}/{y}.jpg', {
      attribution: 'Ortofoto 1943',
    });

    L_OFD_1980 = new L.tileLayer('./tiles/ofd_1980/{z}/{x}/{y}.jpg', {
      attribution: 'Ortofoto 1943',
    });

    L_OFD_1985 = new L.tileLayer('./tiles/ofd_1985/{z}/{x}/{y}.jpg', {
      attribution: 'Ortofoto 1943',
    });

    L_OFD_1998 = new L.tileLayer('./tiles/ofd_1998/{z}/{x}/{y}.jpg', {
      attribution: 'Ortofoto 1943',
    });


    L_OFD_1943.addTo(L_MAP);
    L_OFD_1967.addTo(L_MAP);
    L_OFD_1980.addTo(L_MAP);
    L_OFD_1985.addTo(L_MAP);
    L_OFD_1998.addTo(L_MAP);
...
function STRATI_style(feature) {
    return {
        fillColor: STRATI_getColor(feature.properties.n_strato),
        fillOpacity: 1,
        weight: 1,
        opacity: 1,
        color: STRATI_getColor(feature.properties.n_strato),
        //dashArray: '3',
    };
}
...
function ParticellareEtichette_onEachFeature(feature, layer) {
    param_value = feature.properties["n_part"]
    layer.bindLabel((String(param_value)), {noHide:true});
}
...
//////////// UFOR STILI CLASSI COLTURALI

function UFOR_COLTURA_style(feature) {
    return {
        fillColor: UFOR_COLTURA_getColor(feature.properties.n_coltura),
        fillOpacity: feature.properties.n_coltura == null ?0:0.8 ,
        weight: 1,
        opacity: 1,
        color: 'white'
        //dashArray: '3',
...
function UFOR_COLTURA_getColor(d) {
...
function UFOR_STRATI_style(feature) {
    return {
        fillColor: UFOR_STRATI_getColor(feature.properties.n_strato),
        fillOpacity: feature.properties.n_strato == null ?0:0.8 ,
        weight: 1,
        opacity: 1,
        color: 'white'
        //dashArray: '3',
...
function UFOR_STRATI_getColor(d) {
...
function UFOR_STRATI_2006_style(feature) {
    return {
        fillColor: UFOR_STRATI_2006_getColor(feature.properties.n_strato),
        fillOpacity: feature.properties.n_strato == null ?0:1 ,
        weight: 1,
        opacity: 1,
        color: '#17CAEB'
        //dashArray: '3',
...
function UFOR_goToFeature(e) {
    ...
    UFOR_SELECTED = layer.feature.properties.n_ufor;
    ...
    for (i = 0; i < ufor_comp.features.length; i++) {
        if (ufor_comp.features[i].properties.n_ufor == UFOR_SELECTED){

            r[++j] ='<tr>';
            r[++j] ='<td>';
            r[++j] = ufor_field_decode("id_specie",ufor_comp.features[i].properties.id_specie);
            r[++j] = '</td>';
            r[++j] ='<td>';
            r[++j] = ufor_field_decode("comp_cop_str1",ufor_comp.features[i].properties.cop_str1);
            r[++j] = '</td>';
            r[++j] ='<td>';
            r[++j] = ufor_field_decode("comp_cop_str2",ufor_comp.features[i].properties.cop_str2);
            r[++j] = '</td>';
            r[++j] ='</tr>';
...
/*function UFOR_goToFeature(e) {
    ...
    //var comp_button = '<br><a href="#" id="comp_button" k_ufor="'+layer.feature.properties.n_ufor+'" class="ui-btn ui-shadow ui-corner-all ui-btn-a ui-mini">Composizione</a>';
